...
from nodule_model import unet_nodule
from nodule_model.unet_nodule import Our_nodule
from LUNA16_datasets import myLUNAdataset
from torchvision import transforms
import os
import argparse
import numpy as np
from nodule_model.evaluation import calculate_index
from models.loss_function import MultiClassDiceLoss
from utils.metrics import compute_metrics

# 命令行参数设置
parser = argparse.ArgumentParser()

parser.add_argument('--model', type=str, default='our', choices=['unet','Attention-unet','our','novel-our'])
parser.add_argument('--dataset', type=str, default='myLUNAdata', choices=['myLUNAdata'])
parser.add_argument('--loss', type=str, default='Dice', choices=['Dice'])
...
# 其他准备
BASE_PATH =r'/tmp/pycharm_project_535' if torch.cuda.is_available() else r'D:\Code\lungseg\lungcode\LUNA16\trainingrecords'
...
print('Loading data...')
# 选择数据集
if args.dataset == 'myLUNAdata':
    dataset = myLUNAdataset.Mydataset#修改
else:
    print('数据集异常')
    pass
...
if args.dataset == 'myLUNAdata':
    train_data = dataset(mode='train', transform=transform, target_transform=target_transform,
                         BASE_PATH= r"/tmp/pycharm_project_535/nodule_processed_data/data/train" if torch.cuda.is_available() else r"D:\Code\lungseg\lungcode\nodule_processed_data\data\train")
    val_data = dataset(mode='val', transform=transform, target_transform=target_transform,
                       BASE_PATH=r"/tmp/pycharm_project_535/nodule_processed_data/data/val" if torch.cuda.is_available() else r"D:\Code\lungseg\lungcode\nodule_processed_data\data\val")
else:
    pass
...
# 选择网络模型
if args.model == 'unet':
    net = unet_nodule.UNet_nodule(num_classes = dataset.NUM_CLASSES, in_channels = dataset.CHANNELS_NUM)
elif args.model == 'Attention-unet':
    net = unet_nodule.UNet_nodule(num_classes = dataset.NUM_CLASSES, in_channels = dataset.CHANNELS_NUM, is_acmix = True)
elif args.model == 'our':
    net = unet_nodule.UNet_nodule(num_classes = dataset.NUM_CLASSES, in_channels = dataset.CHANNELS_NUM, is_acmix = True, is_spd = True, is_aspp = True)
...
if args.loss == 'CE':
    ...
    criterion = MultiClassDiceLoss(dataset.NUM_CLASSES)
...
for epoch in range(start_epoch, args.epoch):
    ...
    with torch.no_grad():
        ...
        print(
            'Training: MIoU: {:.4f}, MDSC: {:.4f}, MPC: {:.4f}, AC: {:.4f}, MSE: {:.4f}, MSP: {:.4f}, MF1: {:.4f}'.format(
                miou, mdsc, mpc, ac, mse, msp, mf1
            ))
