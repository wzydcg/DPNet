...
import numpy as np
from models import unet_fpn,unet  # 假设这是你的分割模型
...
class MultiTaskModel(nn.Module):
    ...
    def forward(self, x):
        # 分割输出
        seg_output = self.seg_model(x)

        # 分类输出
        cls_output = self.cls_model(x)
...
# 训练函数（同时训练分割和分类）
def train_epoch_multi_task(seg_model, cls_model, loader, seg_criterion, cls_criterion,
                           seg_optimizer, cls_optimizer, cls_weight=0.5):
    seg_model.train()
    cls_model.train()
...
# 验证函数（同时验证分割和分类）
def validate_multi_task(seg_model, cls_model, loader, seg_criterion, cls_criterion, cls_weight=0.5):
    seg_model.eval()
    cls_model.eval()
...
if __name__ == '__main__':
    ...
    # 初始化分割模型 (UNet)
    seg_model = unet.UNet(num_classes=NUM_CLASSES_SEG, in_channels=3, is_esa= False).to(device)

    # 初始化分类模型 (ResNet50)
    cls_model = ClassificationModel(num_classes=NUM_CLASSES_CLS).to(device)

    # 创建多任务模型（用于测试输出形状）
    multi_task_model = MultiTaskModel(seg_model, cls_model).to(device)
    ...
    cls_scheduler = optim.lr_scheduler.CosineAnnealingLR(cls_optimizer, T_max=EPOCHS)
    ...
    for epoch in range(EPOCHS):
        ...
        train_total_loss, train_seg_loss, train_cls_loss, train_seg_acc, train_cls_acc = \
            train_epoch_multi_task(seg_model, cls_model, train_loader, seg_criterion, cls_criterion,
                                   seg_optimizer, cls_optimizer, CLS_WEIGHT)
        ...
        val_total_loss, val_seg_loss, val_cls_loss, val_seg_acc, val_dice, val_iou, val_cls_acc = \
            validate_multi_task(seg_model, cls_model, val_loader, seg_criterion, cls_criterion, CLS_WEIGHT)
        ...
        torch.save({
            ...
            'cls_optimizer_state_dict': cls_optimizer.state_dict(),
        ...
        if combined_score > best_combined_score:
            ...
            print(f"✓ 保存最佳模型: {best_model_path} (Dice: {val_dice:.4f}, 分类准确率: {val_cls_acc:.4f})")
    ...
    def test_output_shape():
        ...
        with torch.no_grad():
            seg_output, cls_output = multi_task_model(sample_input)
        print(f"\n输出形状验证:")
